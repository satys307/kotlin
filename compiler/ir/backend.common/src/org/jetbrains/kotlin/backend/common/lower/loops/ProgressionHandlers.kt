...
internal class StepHandler(
    ...
    override fun build(expression: IrCall, data: ProgressionType, scopeOwner: IrSymbol): HeaderInfo? =
        with(context.createIrBuilder(scopeOwner, expression.startOffset, expression.endOffset)) {
            ...
            val stepGreaterFun = context.irBuiltIns.greaterFunByOperandType.getValue(stepType.classifierOrFail)
    ...
    private fun DeclarationIrBuilder.callGetProgressionLastElementIfNecessary(
        ...
        val returnType = if (progressionType.isUnsigned) {
            progressionType.elementType(symbols).toKotlinType()
        } else {
            progressionType.stepType(context.irBuiltIns).toKotlinType()
        }
        val getProgressionLastElementFun = symbols.getProgressionLastElementByReturnType[returnType]
            ?: throw IllegalArgumentException("No `getProgressionLastElement` for return type $returnType")
